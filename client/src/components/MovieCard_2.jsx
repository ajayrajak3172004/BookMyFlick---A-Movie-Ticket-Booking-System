...
export default function MovieCard_2({movie}) {

    const navigate = useNavigate()
    return (
        <>
            <div className={` w-[15rem]  sm:w-[18rem]   flex justify-center items-center p-3 bg-gray-900/60 my-2 rounded-2xl  transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg`}>
                <div className='w-[100%] h-auto text-white'>
                    <img onClick={()=>{navigate(`/movies/${movie._id}`);scroll(0,0)}} src={movie.backdrop_path} className='w-full  sm:h-[18rem]   my-4 cursor-pointer'></img>
                    <h1 className=' text-xl  text-bold'>{movie.title}</h1>
